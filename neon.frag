...
void main() {
  ...
  // Recover naturally bright detail already present in the nebula.
  ...
  // Select colourful regions, not grey/white haze.
  ...
  // Make existing bright filaments glow in their own colour.
  vec3 filament = pow(max(source, 0.0), vec3(2.2));
  col += filament * detail * selected * 0.75;

  // Sparse star-like sparkles, anchored to image coordinates.
  ...
  // Keep colour saturated and prevent the haze from becoming white.
